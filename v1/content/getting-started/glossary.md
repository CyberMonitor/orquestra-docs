...
- `Steps`: Steps consists of a collection of tasks for execution by the quantum engine
- `Task`: An execution process in a workflow - performed by invoking a template.
...
- `Parameter`: A simple variable (e.g. string, integer, float, etc) that is typically passed to a component and can be configured inside of a workflow.
- `Workflow Parameter`: A parameter which can be referred to anywhere inside of the workflow. 
- `Entrypoint`: The component that is called when the workflow begins.

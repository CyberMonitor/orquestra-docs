...
This tutorial will walk through an implementation of a neural network model that is able to learn how the reconstruct the shape a noisy sine function using *Recurrent Neural Networks*.
...
The problem here considered belongs in the category of *Supervised Learning*, which means learning a function to maps a set of inputs to one of outputs based on multiple instances of (input, output) pairs. In the folowing section it is stated the input and output datasets of the problem here at hand.


## Input and Output Dataset
The raw dataset consists of a *noisy sine function* data, generated by adding up an analytical sine function and a noisy signal sampled from a normal distribution with mean zero and standard deviation of one.above the noise follows a normal distribution centered at zero.  The parameters that control the data generation are the folowing:
...
- *noise-std* : the standard deviation of the noise. As mentioned 
...
where the **x**'s represent the set of values of size *lookback window* that corresponds to the input data, and the **h**'s are the state values of the neurons in the RNN layer, which summarise the information from all the previous steps within the *lookback window* seen thus far. Last, **y** represents the output data to be predicted.

Therefore, the idea of a NN model as a computational black box can be substantiated looking at the NN model as a machine a accepting a series of inputs, **x**'s, to produce an output **y**.

The model here used consists of a neural network with a single hidden layer of *LSTM* neurons and a final *Dense* layer for the output, where *Dense* in this context means that all neurons from the previous layer are connected to the current one, in this case the output layer with one single neuron, which corresponds to the **y** predicted value. Additionally, in order to prevent overfitting in the training process a *Dropout* operation is added to the LSTM layer, which can be seen as a regularizer.
...
After having created the model, it is possible to print out its main characteristics, as layers and number of corresponding parameters, by means of the keras command `{your_model_name}.summary()`, which summarizes the model as shown below:

![](../img/Model_Summary.png)

It is also possible to plot the model by means of importing package `from tensorflow.keras.utils import plot_model`, and command `plot_model({your_model_name}, to_file='{plot_name}.png', `
`how_shapes=True, show_layer_names=False)`, which output will look as follows:
...
Now we will add the bulk of the workflow, which is the Python code that trains the model. For this purpose, using either the GitHub UI or by cloning your repo and using the command line, create two files: `src/python/lstm/data_manipulator.py` to generate and manipulate the input datasets, and `src/python/lstm/lstm_model.py` to build the network model. Their corresponding code are stated below:
...
Create file `lstm-tutorial.yaml` (with code shown below), inserting the URL of your github repository in line 15. This file can go anywhere (except the `templates` folder), and in the repo you'll find it under `example`.

This yaml file orquestrates all the different steps corresponding to data generation and preprocessing, and the buiding up and training of the model, ending up with running the model prediction on the test dataset.
...
Now all should be ready to actually run the workflow.
...
In order to plot the result from the training and testing processes it can be used the file `plot_lstm.py`. In the repo, it can be found in the folder named `example`. This file plots the results from the existing final `json` file generated by the workflow above, and it can be run passing the json file as a parameter after the pyhton script, i.e. `python3 plot_lstm.py <you_own_json_file>`. 
...
Finally, zooming in on the last part of the plot above it is shown just a window of the whole dataset focusing on the test data and predicted values.

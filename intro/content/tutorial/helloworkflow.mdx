...
Now that we've built the code that we want the resource to execute, we'll now work defining the `welcome` template for this resource. Move into the `templates/` folder and create a new file called `welcome.yaml`.

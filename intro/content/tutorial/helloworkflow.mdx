...
2. It can reference code that will take in those inputs and generate outputs.
3. It can contain a list of templates.
...
A. Create one or more templates and associated source code (optional)
B. Make available templates as an resource (optional)
C. Incorporate the resource into a workflow

Note: A and B are optional if you reuse existing resources.
...
This repository will be your resource. To be used by Orquestra, resources must contain two folders:
...
Our goal in this tutorial is to create a resource that produces an [artifact](https://www.orquestra.io/docs/dcs/data/artifacts/) with the text `Welcome to Orquestra!` when used within a workflow. The contents of `welcome.py` that will achieve this are below:
...
*Important note*: We create the file `welcome.json` because any output has to be saved in a file in order to be passed on to the next template or to be accessible after the workflow has completed. We will see how to pass it on to another template later in this tutorial. All artifacts must also have a `schema` field. More details on artifacts are found on the [artifacts page](https://www.orquestra.io/docs/dcs/data/artifacts/).
...
For example, Zapata has a resource with a URL of `git@github.com:zapatacomputing/ztransform.git`. This will transform a message you give it in a fun way.
...
- name: ztransform
  type: git
  parameters:
    url: "git@github.com:zapatacomputing/ztransform.git"
    branch: "master"
```
...
- name: welcome
  type: git
  parameters:
    url: "git@github.com:zapatacomputing/welcome.git"
    branch: "master"

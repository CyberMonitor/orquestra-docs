...
In this example, `name` is the name of your package. The `packages` field in this example tells the installer to look in your repo in a folder called `python` for your package. The `package_dir` field in this sample means that the contents of the "python" folder can be imported without the python path prefix, for example, with `from orquestra import all`.
...
The `welcome.yaml` template is responsible for invoking Python code that lives under `src/`. This can be done using the following snippet:

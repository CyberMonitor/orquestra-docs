...
1. Login to quantum engine.
2. Download the [hydrogen VQE example workflow](/hydrogen.yaml). (Right click and select `Save Link As`.)
3. Submit the workflow with `qe submit workflow hydrogen.yaml`.
4. Monitor the status of your workflow with `qe get workflow workflow_id` where
`workflow_id` is the ID of your workflow.
5. After the workflow has completed, run `qe get workflowresult workflow_id` to request the workflow results.
6. Continue to run `qe get workflowresult workflow_id` until a URL with workflow results is returned.
7. Open the URL to download the JSON file with the workflow results.
8. Copy the python script below into the same directory as the JSON file.
9. In the python script, replace the name of the JSON file with the name of the JSON file that you have downloaded in Step 7.
10. Run the python script to plot the calculated binding energy curve.

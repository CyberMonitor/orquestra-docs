...
Next, we will see how to use a pre-existing resource and explore one possible pattern for multiple steps.
...
- There is now a second step with a `name` of `bye`. `bye` calls the `template` named `say-bye`. This template is not defined in either of the resources. It is defined later in the workflow.
...
This pattern of calling a template that has a list of steps allows you to make groupings of steps under a single name that can be called in your workflow. This can be useful for organization, and it allows you to easily reuse a sequence of steps.
...
**12. Log in to Quantum Engine by running `qe login -e <your-email> -s <quantum-engine-uri>` in your command line. Contact support to register your email or receive the `quantum-engine-uri`.**

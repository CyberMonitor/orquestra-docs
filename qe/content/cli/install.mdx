...
3. (Optional) Move `qe.exe` to the `%SystemRoot%\system32` folder or add `qe.exe` to your PATH 
4. Open the command prompt (`cmd` or `powershell`) and use `qe`
...
2. In your terminal navigate to the downloaded binary and rename it `mv ./qe-linux-amd64 ./qe`
3. Make the binary executable `chmod +x ./qe`
4. Copy the binary to a general location `sudo cp ./qe /usr/local/bin/`

### Notes
If the above scripts do not work, you can download `qe` from [github](https://github.com/zapatacomputing/qe-cli/releases/latest) as an binary. There are individual download links for common platforms.*